...
const ChatBot = ({ setCurriculum, resetCurriculum }) => {
  ...
  const handleExplorerCurrentKnowledge = (answer) => {
  ...
  const displayCurriculum = async (curriculum) => {
  ...
  const handlePathwaysOccupation = (answer) => {
  ...
  const handlePathwaysGoal = (answer) => {
    setUserProfile({ ...userProfile, goal: answer })
  ...
  const displaySuggestedAreas = (areas) => {
    const areasDisplay = (
      <div>
